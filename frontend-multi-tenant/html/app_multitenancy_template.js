  ...
  user ? handleSignedInUser(user) : handleSignedOutUser();
          ...
          if (user) {
            ...
            user.getIdToken().then(function(accessToken) {
              ...
              // Depends on https://github.com/auth0/jwt-decode
              document.getElementById('infopanel').textContent = JSON.stringify(jwt_decode(accessToken), null, 2);
            });
          }
...
var initApp = function() {
  var tenant_select_button = document.getElementsByClassName('tenant-select-button')
  for (var i = 0; i < tenant_select_button.length; i++) {
    tenant_select_button[i].addEventListener('click', function (e) {
      var tenantId = e.target.getAttribute('data-val');
      console.log('tenant:', tenantId)
      firebase.auth().tenantId = tenantId;
      document.getElementById("current-tenant").innerHTML = tenantId;
      ui.reset();
  ...
  document.getElementById('test-api1').addEventListener(
      'click', function() {
          $.ajax({
              url: "https://api-gw-52akcbtoya-ew.a.run.app/hello",
              headers: { 'Authorization': 'Bearer '+id_token },
              success: function(data, status){
                console.log(data);
                $('#infopanel').text(JSON.stringify(data, null, 2))
              }
            })
      });

  document.getElementById('test-api2').addEventListener(
      'click', function() {
          $.ajax({
